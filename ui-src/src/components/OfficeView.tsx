...
const HALLWAY_H = 32;
const TARGET_CHAR_H = 54.6; // Increased by 5% from 52
...
export default function OfficeView({
  ...
  // Track agent sprite positions for interaction
  const agentPositionsRef = useRef<Map<string, { x: number; y: number; agent: Agent }>>(new Map());
  ...
  const activeTextsRef = useRef<Text[]>([]);

  const getPooledTextStyle = (options: any) => {
    const key = JSON.stringify(options);
    if (textStyleCacheRef.current.has(key)) return textStyleCacheRef.current.get(key)!;

    // Safety: prevent cache from growing indefinitely - ultra strict for 200MB limit
    if (textStyleCacheRef.current.size > 50) {
      console.warn("[Memory] Clearing textStyleCache due to size limit (50)");
      textStyleCacheRef.current.clear();
    }

    const style = new TextStyle(options);
    textStyleCacheRef.current.set(key, style);
    return style;
  };

  const getPooledText = (text: string, styleOptions: any) => {
    const style = getPooledTextStyle(styleOptions);
    while (textPoolRef.current.length > 0) {
      const candidate = textPoolRef.current.pop()!;
      if (!candidate.destroyed && (candidate as any).scale && (candidate as any).anchor) {
        const textInstance = candidate;
        textInstance.text = text;
        textInstance.style = style;
        textInstance.alpha = 1;
        textInstance.visible = true;
        textInstance.tint = 0xffffff;
        textInstance.rotation = 0;
        textInstance.scale.set(1);
        textInstance.anchor.set(0, 0);
        activeTextsRef.current.push(textInstance);
        return textInstance;
      }
    }
    const textInstance = new Text({ text, style });
    activeTextsRef.current.push(textInstance);
    return textInstance;
  };

  const recycleTexts = () => {
    for (const t of activeTextsRef.current) {
      if (t.destroyed) continue;
      t.parent?.removeChild(t);
      t.visible = false;
      if (textPoolRef.current.length < 100) {
        textPoolRef.current.push(t);
      } else {
        t.destroy();
      }
    }
  ...
  const triggerDepartmentInteract = useCallback(() => {
    ...
    console.log(`[Interact] Attempting interact at CEO position (${cx}, ${cy})`);

    // First check for nearby agents using tracked positions (higher priority)
    const AGENT_INTERACT_DISTANCE = 150; // Increased to 150 for much easier detection
    let closestAgent: { agent: Agent; distance: number } | null = null;

    for (const [agentId, pos] of agentPositionsRef.current.entries()) {
      const distance = Math.sqrt(Math.pow(cx - pos.x, 2) + Math.pow(cy - pos.y, 2));
      console.log(`[Interact] Agent ${agentId} at (${pos.x}, ${pos.y}), distance: ${distance.toFixed(1)}`);

      if (distance < AGENT_INTERACT_DISTANCE) {
        if (!closestAgent || distance < closestAgent.distance) {
          closestAgent = { agent: pos.agent, distance };
        }
      }
    }

    if (closestAgent) {
      console.log(`[Interact] ✓ Triggered agent: ${closestAgent.agent.id} (distance: ${closestAgent.distance.toFixed(1)})`);
      cbRef.current.onSelectAgent(closestAgent.agent);
      return;
    }

    // Then check for departments
    for (const r of roomRectsRef.current) {
      // Expanded hitbox logic: Allows interaction anywhere near or inside the room.
      if (cx >= r.x - 20 && cx <= r.x + r.w + 20 && cy >= r.y - 30 && cy <= r.y + r.h + 30) {
        console.log(`[Interact] ✓ Triggered department: ${r.dept.id}`);
        cbRef.current.onSelectDepartment(r.dept);
        return;
      }
    }
    console.log("[Interact] ✗ No department or agent in range.");
  }, []);

  const interactRef = useRef(triggerDepartmentInteract);
  interactRef.current = triggerDepartmentInteract;

  const setMoveDirectionPressed = useCallback((direction: MobileMoveDirection, pressed: boolean) => {
    console.log(`[VirtualPad] ${direction} -> ${pressed}`);
  ...
  const clearVirtualMovement = useCallback(() => {
    console.log("[Movement] Clearing all virtual inputs");
  ...
  useEffect(() => {
    const updateVirtualPadVisibility = () => {
      ...
      // Also show if we are in full screen or maximized (heuristically check if browser is mostly filling the screen)
      const isFullScreen = !!(document.fullscreenElement || (window.innerWidth === window.screen.width && window.innerHeight === window.screen.height));
      const isMaximized = (window.innerWidth >= window.screen.availWidth * 0.75 && window.innerHeight >= window.screen.availHeight * 0.6);
      setShowVirtualPad(isCoarsePointer || isNarrowViewport || isFullScreen || isMaximized);
    };
  ...
  // Removed intrusive clearVirtualMovement useEffect to prevent input loss during layout shifts
  ...
  const buildScene = useCallback(() => {
    ...
    try {
      console.log("[BuildScene] Starting build...");

      // Clear agent positions for fresh tracking
      ...
      for (const child of oldChildren) {
        ...
        if (child instanceof Text && activeTextsRef.current.includes(child)) continue;
      ...
      const ceoLabel = getPooledText(
        pickLocale(activeLocale, LOCALE_TEXT.ceoOffice),
        { fontSize: 10, fill: 0xffffff, fontWeight: "bold", fontFamily: "monospace", letterSpacing: 2 },
      );
      ...
      const ceoPlateText = getPooledText("CEO", { fontSize: 5, fill: 0x000000, fontWeight: "bold", fontFamily: "monospace" });
      ...
      const meetingLabel = getPooledText(
        pickLocale(activeLocale, LOCALE_TEXT.collabTable),
        { fontSize: 7, fill: 0x7a5c2a, fontWeight: "bold", fontFamily: "monospace", letterSpacing: 1 },
      );
      ...
      stats.forEach((s, i) => {
        ...
        const ti = getPooledText(s.icon, { fontSize: 10 });
        ti.position.set(sx + 4, sy + 4);
        ceoLayer.addChild(ti);
        ceoLayer.addChild(Object.assign(getPooledText(s.label, { fontSize: 7, fill: 0x8b7040, fontFamily: "monospace" }), { x: sx + 18, y: sy + 2 }));
        ceoLayer.addChild(Object.assign(getPooledText(s.val, { fontSize: 10, fill: 0x5a4020, fontWeight: "bold", fontFamily: "monospace" }), { x: sx + 18, y: sy + 13 }));
      });

      // Keyboard hint
      const hint = getPooledText(
        pickLocale(activeLocale, LOCALE_TEXT.hint),
        {
          fontSize: 10,
          fontWeight: "bold",
          fill: 0x8b7040,
      ...
      if (activeMeetingTaskIdRef.current) {
        const meetingHint = getPooledText(
          pickLocale(activeLocale, LOCALE_TEXT.meetingTableHint),
          {
            fontSize: 12,
            fill: 0x8b6b30,
            fontWeight: "bold",
      ...
      departments.forEach((dept, deptIdx) => {
        ...
        const signTxt = getPooledText(
          `${dept.icon || "🏢"} ${activeLocale === "ko" ? (dept.name_ko || dept.name) : dept.name}`,
          { fontSize: 9, fill: 0xffffff, fontWeight: "bold", fontFamily: "system-ui, sans-serif", dropShadow: { alpha: 0.2, distance: 1, color: 0x000000 } },
        );
        ...
        if (deptAgents.length === 0) {
          const et = getPooledText(
            pickLocale(activeLocale, LOCALE_TEXT.noAssignedAgent),
            { fontSize: 10, fill: 0x9a8a7a, fontFamily: "system-ui, sans-serif" },
          );
        ...
        deptAgents.forEach((agent, agentIdx) => {
          ...
          const nt = getPooledText(
            activeLocale === "ko" ? (agent.name_ko || agent.name) : agent.name,
            { fontSize: 7, fill: 0x3a3a4a, fontWeight: "bold", fontFamily: "system-ui, sans-serif" },
          );
          nt.anchor.set(0.5, 0);
          ...
          if (unread?.has(agent.id)) {
            ...
            const bangTxt = getPooledText("!", { fontSize: 8, fill: 0xffffff, fontWeight: "bold", fontFamily: "monospace" });
          ...
          const rt = getPooledText(
            ...
            { fontSize: 6, fill: contrastTextColor(theme.accent), fontFamily: "system-ui, sans-serif" },
          );
          ...
          if (isBreak) {
            ...
            const awayTag = getPooledText(
              pickLocale(activeLocale, LOCALE_TEXT.breakRoom),
              {
                fontSize: 8,
            ...
            // Track agent position for keyboard interaction
            agentPositionsRef.current.set(agent.id, { x: ax, y: charFeetY, agent });
            ...
            if (frames.length > 0) {
              ...
              const fb = getPooledText(agent.avatar_emoji || "🤖", { fontSize: 24 });
              fb.anchor.set(0.5, 1);
            ...
            if (activeTask) {
              ...
              const bt = getPooledText(`💬 ${txt}`, { fontSize: 6.5, fill: 0x333333, fontFamily: "system-ui, sans-serif", wordWrap: true, wordWrapWidth: 85 });
              bt.anchor.set(0.5, 1);
            ...
            if (isOffline) {
              const zzz = getPooledText("💤", { fontSize: 12 });
          ...
          mySubs.forEach((sub, si) => {
            ...
            room.addChild(abBg);
            const abTxt = getPooledText(
              pickLocale(activeLocale, LOCALE_TEXT.partTime),
              { fontSize: 6, fill: 0x000000, fontWeight: "bold", fontFamily: "system-ui, sans-serif" },
            );
      ...
      breakAgents.forEach((agent, idx) => {
        ...
        // Track agent position for keyboard interaction
        agentPositionsRef.current.set(agent.id, { x: spotX, y: spotY, agent });
        ...
        if (tex) {
          ...
          const fb = getPooledText(agent.avatar_emoji || "🤖", { fontSize: 20 });
          fb.anchor.set(0.5, 1);
          charContainer.addChild(fb);
        }
        ...
        const coffeeEmoji = getPooledText("☕", { fontSize: 10 });
        coffeeEmoji.anchor.set(0.5, 0.5);
        coffeeEmoji.position.set(spotX + 14, spotY - 10);
        breakRoom.addChild(coffeeEmoji);

        // Small name tag
        const nameTag = getPooledText(
          activeLocale === "ko" ? (agent.name_ko || agent.name) : agent.name,
          { fontSize: 6, fill: 0x4a3a2a, fontFamily: "system-ui, sans-serif" },
        );
      ...
      if (breakAgents.length > 0) {
        ...
        for (let si = 0; si < speakerCount; si++) {
          ...
          const bubbleText = getPooledText(msg, { fontSize: 7, fill: 0x333333, fontFamily: "system-ui, sans-serif" });
      ...
      if (textures["ceo"] && textures["ceo"].width > 0 && textures["ceo"].height > 0) {
        const sp = new Sprite(textures["ceo"]);
        sp.anchor.set(0.5, 0.5);
        const maxDim = Math.max(1, sp.texture.width, sp.texture.height);
        const s = CEO_SIZE / maxDim;
        // Avoid scaling to Infinity if something goes wrong
      ...
      crown.position.set(0, -CEO_SIZE * 0.3 - 2);
      ceoChar.addChild(crown);
      crownRef.current = crown;

      // CEO name badge
      const cbg = new Graphics();
      cbg.roundRect(-16, CEO_SIZE * 0.3 + 1, 32, 11, 3).fill({ color: 0xf0d888, alpha: 0.9 });
      ...
      cName.position.set(0, CEO_SIZE * 0.3 + 6.5);
      ceoChar.addChild(cName);

      ceoChar.position.set(ceoPosRef.current.x, ceoPosRef.current.y);
      ceoChar.eventMode = "static";
      ceoChar.cursor = "pointer";
      ceoChar.on("pointerdown", (e) => {
        e.stopPropagation();
        interactRef.current();
      });
      ...
      console.log("[BuildScene] Build complete.");
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      await app.init({
        ...
        backgroundAlpha: 0,
        antialias: false,
        resolution: Math.min(window.devicePixelRatio || 1, 1.25), // Reduced from 1.5 to 1.25 to save memory
        autoDensity: true,
        preference: 'webgl', // Force WebGL instead of WebGPU for better compatibility
        powerPreference: 'low-power', // Use low-power mode to reduce memory usage
        clearBeforeRender: true, // Clear canvas each frame to prevent memory buildup
      });
      ...
      loads.push(Assets.load<Texture>("/sprites/ceo-crab-user.png").then(t => { textures["ceo"] = t; }).catch(() => { }));
      ...
      try {
        ...
        console.log("[Init] buildScene done, initDone = true");
        followCeoInView();
      } catch (err) {
        console.error("[Init] Error in initial buildScene:", err);
      }

      // ── ANIMATION TICKER ──
      console.log("[Init] Adding ticker...");
      app.ticker.maxFPS = 60; // Keep 60 FPS for smooth animation
      app.ticker.add(() => {
        if (destroyedRef.current) return;
        if (appRef.current !== app) {
          // This happens if the component re-initialized and this ticker belongs to an old app
          return;
        }
        const tick = ++tickRef.current;

        // Memory safety: limit total particles across all containers
        let totalParticles = 0;
        for (const { particles } of animItemsRef.current) {
          totalParticles += particles.children.length;
        }
        const steamC = breakSteamParticlesRef.current;
        if (steamC) totalParticles += steamC.children.length;

        // If too many particles, skip particle generation this frame
        const particleLimitReached = totalParticles > 180;
        if (tick % 120 === 0) console.log(`[Ticker] Healthy. appRef Match: ${appRef.current === app}`);
        ...
        if (ceo) {
          ...
          if (dx || dy) {
            console.log(`[Movement] dx:${dx} dy:${dy} pos:(${ceoPosRef.current.x}, ${ceoPosRef.current.y})`);
        ...
        for (const { sprite, status, baseX, baseY, particles, agentId, cliProvider, deskG, bedG, blanketG } of animItemsRef.current) {
          ...
          if (agentId) {
            ...
            const inMeeting = inMeetingPresence || deliveriesRef.current.some(
              d => d.agentId === agentId && (d.holdAtSeat || d.type === "walk"),
            );
          ...
          if (status === "working") {
            if (!particleLimitReached && tick % 23 === 0) { // 25 * 0.93 ≈ 23 (7% faster)
            ...
            // Safety: force cleanup if too many particles in this container
            if (particles.children.length > 30) {
              for (let i = particles.children.length - 1; i >= 30; i--) {
          ...
          if (cliProvider) {
            ...
            if (maxUtil >= 1.0) {
              ...
              if (!particleLimitReached && tick % 56 === 0) { // 60 * 0.93 ≈ 56 (7% faster)
              ...
              if (!particleLimitReached && tick % 112 === 0) { // 120 * 0.93 ≈ 112 (7% faster)
                const zz = new Text({
                  text: "z",
              ...
              if (bedG) bedG.visible = false;
              if (blanketG) blanketG.visible = false;
              if (!particleLimitReached && tick % 74 === 0) { // 80 * 0.93 ≈ 74 (7% faster)
                const drop = new Graphics();
                drop.moveTo(0, 0).lineTo(-1.8, 4).quadraticCurveTo(0, 6.5, 1.8, 4).lineTo(0, 0)
                  .fill({ color: 0x7ec8e3, alpha: 0.85 });
                drop.circle(0, 3.8, 1.2).fill({ color: 0xbde4f4, alpha: 0.5 });
            ...
            // Safety: force cleanup if too many sweat particles
            if (particles.children.length > 30) {
              for (let i = particles.children.length - 1; i >= 30; i--) {
                const p = particles.children[i];
                particles.removeChild(p);
        ...
        if (steamC) {
          if (!particleLimitReached && tick % 37 === 0) { // 40 * 0.93 ≈ 37 (7% faster)
            const p = new Graphics();
          ...
          // Safety: force cleanup if too many steam particles
          if (steamC.children.length > 20) {
            for (let i = steamC.children.length - 1; i >= 20; i--) {
              const p = steamC.children[i];
    ...
    return () => {
      ...
      // Clean up deliveries
      for (const d of deliveriesRef.current) {
        if (d.sprite && !d.sprite.destroyed) {
          d.sprite.destroy({ children: true, texture: false });
        }
      }
      deliveriesRef.current = [];

      // Clean up text pool
      for (const t of textPoolRef.current) {
        if (t && !t.destroyed) {
          t.destroy({ children: true, texture: false });
        }
      }
      textPoolRef.current = [];
      ...
      if (appRef.current) {
        try {
          ...
          // Clear stage children manually to avoid linked list errors
          ...
          // Destroy app without destroying children (already done above)
          appRef.current.destroy(false, {
            children: false,
            texture: false
          });
        } catch (e) {
          console.error('[Cleanup] Error destroying PixiJS app:', e);
        }
        appRef.current = null;
      }

      // Clear texture cache (shared across all PixiJS instances)
      texturesRef.current = {};
    };
  }, [buildScene, triggerDepartmentInteract, followCeoInView]);

  /* ── REBUILD SCENE on data change (with debounce to prevent OOM) ── */
  useEffect(() => {
    if (!initDoneRef.current || !appRef.current) return;

    const timer = setTimeout(() => {
      buildScene();
    }, 150); // Small debounce to batch rapid data updates
  ...
  useEffect(() => {
    ...
    for (const row of rows) {
      ...
      dc.addChild(badge);
      const badgeText = getPooledText("", { fontSize: 7, fill: 0x111111, fontWeight: "bold", fontFamily: "system-ui, sans-serif" });
    ...
    for (let i = deliveriesRef.current.length - 1; i >= 0; i--) {
      ...
      // Meeting ended. Walk back to desk instead of disappearing.
      const deskPos = agentPosRef.current.get(d.agentId);
      if (deskPos) {
        d.fromX = d.sprite.x;
        d.fromY = d.sprite.y;
        d.toX = deskPos.x;
        ...
        d.holdAtSeat = false;
        d.type = "walk";
        d.speed = 0.005; // standard walk speed
        // Badge is usually hidden during normal walk? 
        // Or keep it for continuity but change label? Let's hide it for "going back".
        if (d.badgeGraphics) d.badgeGraphics.visible = false;
        if (d.badgeText) d.badgeText.visible = false;
      } else {
        destroyNode(d.sprite);
  ...
  useEffect(() => {
    ...
    for (const cd of crossDeptDeliveries) {
      ...
      const docEmoji = getPooledText("📋", { fontSize: 13 });
      docEmoji.anchor.set(0.5, 0.5);
      ...
      dc.addChild(badge);
      const badgeText = getPooledText(
        pickLocale(language, LOCALE_TEXT.collabBadge),
        { fontSize: 7, fill: 0x000000, fontWeight: "bold", fontFamily: "system-ui, sans-serif" },
      );
      badgeText.anchor.set(0.5, 0.5);
  ...
  useEffect(() => {
    ...
    const renderSpeechBubble = (x: number, y: number, phase: "kickoff" | "review", line: string) => {
      ...
      const bubbleText = getPooledText(line, {
        fontSize: 7,
        fill: 0x2b2b2b,
        fontFamily: "system-ui, sans-serif",
        wordWrap: true,
  ...
  const mobilePadButtonClass =
    "pointer-events-auto flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-lg font-medium text-white/90 shadow-2xl backdrop-blur-xl transition-all hover:bg-white/10 hover:border-white/20 active:scale-90 active:bg-white/15";
  ...
  return (
    <div className="w-full overflow-auto" style={{ minHeight: "100%" }}>
      <div className="relative mx-auto w-full">
        ...
        {showVirtualPad && (
          <>
            <div className="fixed bottom-10 left-1/2 z-[10000] -translate-x-1/2 pointer-events-auto">
              <button
                type="button"
                aria-label="Interact"
                className="flex h-12 min-w-[100px] items-center justify-center rounded-2xl border border-cyan-400/30 bg-cyan-500/15 px-6 text-[12px] font-bold tracking-[0.2em] uppercase text-cyan-50 shadow-[0_0_25px_rgba(34,211,238,0.25)] backdrop-blur-xl transition-all hover:bg-cyan-500/25 hover:border-cyan-400/50 active:scale-95 active:bg-cyan-400/40"
            ...
            <div className="fixed bottom-10 right-10 z-[10000] pointer-events-auto">
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  style={{ touchAction: "none" }}
                  onPointerDown={(e) => { e.preventDefault(); setMoveDirectionPressed("up", true); }}
                  onPointerUp={(e) => { e.preventDefault(); setMoveDirectionPressed("up", false); }}
                  onPointerCancel={(e) => { e.preventDefault(); setMoveDirectionPressed("up", false); }}
                >
                ...
                <button
                  ...
                  className={mobilePadButtonClass}
                  style={{ touchAction: "none" }}
                  onPointerDown={(e) => { e.preventDefault(); setMoveDirectionPressed("left", true); }}
                  onPointerUp={(e) => { e.preventDefault(); setMoveDirectionPressed("left", false); }}
                  onPointerCancel={(e) => { e.preventDefault(); setMoveDirectionPressed("left", false); }}
                >
                ...
                <button
                  ...
                  className={mobilePadButtonClass}
                  style={{ touchAction: "none" }}
                  onPointerDown={(e) => { e.preventDefault(); setMoveDirectionPressed("right", true); }}
                  onPointerUp={(e) => { e.preventDefault(); setMoveDirectionPressed("right", false); }}
                  onPointerCancel={(e) => { e.preventDefault(); setMoveDirectionPressed("right", false); }}
                >
                ...
                <button
                  ...
                  className={mobilePadButtonClass}
                  style={{ touchAction: "none" }}
                  onPointerDown={(e) => { e.preventDefault(); setMoveDirectionPressed("down", true); }}
                  onPointerUp={(e) => { e.preventDefault(); setMoveDirectionPressed("down", false); }}
                  onPointerCancel={(e) => { e.preventDefault(); setMoveDirectionPressed("down", false); }}
                >
