...
$logoPath = Join-Path $scriptDir "logo.txt"
...
if (!(Test-Path "package.json") -or !(Test-Path "scripts/setup.mjs")) {
  throw "Run this script from the Claw-NexusClaw repository."
}
...
Write-Host "[Claw-NexusClaw] Installing dependencies..."
pnpm install

if (-not (Test-Path ".env")) {
  Copy-Item ".env.example" ".env"
  Write-Host "[Claw-NexusClaw] Created .env from .env.example"
}
...
if ([string]::IsNullOrWhiteSpace($OpenClawConfig)) {
  ...
  if (Test-Path $candidate) {
    ...
    Write-Warning "[Claw-NexusClaw] OPENCLAW config not found at $candidate. Keeping path for later."
...
Write-Host "[Claw-NexusClaw] Installing AGENTS.md orchestration rules..."
& pnpm @setupArgs

Write-Host ""
Write-Host "[Claw-NexusClaw] Setup complete."
Write-Host "Frontend: http://127.0.0.1:5173"
Write-Host "API:      http://127.0.0.1:$portToUse/healthz"

if ($Start) {
  Write-Host "[Claw-NexusClaw] Starting development server..."
  pnpm dev:local
  exit $LASTEXITCODE
}
