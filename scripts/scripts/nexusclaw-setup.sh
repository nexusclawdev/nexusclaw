...
usage() {
  cat <<'EOF'
Claw-NexusClaw one-click setup (macOS/Linux)
...
# Display NexusClaw logo
LOGO_PATH="${SCRIPT_DIR}/logo.txt"
if [[ -f "${LOGO_PATH}" ]]; then
  echo ""
  cat "${LOGO_PATH}"
  echo ""
...
if [[ ! -f package.json || ! -f scripts/setup.mjs ]]; then
  echo "Run this script from the Claw-NexusClaw repository." >&2
  exit 1
fi
...
echo "[Claw-NexusClaw] Installing dependencies..."
pnpm install

if [[ ! -f .env ]]; then
  cp .env.example .env
  echo "[Claw-NexusClaw] Created .env from .env.example"
fi
...
if [[ -z "${OPENCLAW_CONFIG}" ]]; then
  ...
  if [[ ! -f "${OPENCLAW_CONFIG}" ]]; then
    echo "[Claw-NexusClaw] Warning: OPENCLAW config not found at ${OPENCLAW_CONFIG}. Keeping path for later."
  fi
fi
...
if (!currentSecret || currentSecret === "__CHANGE_ME__") {
  ...
  console.log("[Claw-NexusClaw] Generated OAUTH_ENCRYPTION_SECRET");
}
...
if (!currentInboxSecret || currentInboxSecret === "__CHANGE_ME__") {
  ...
  console.log("[Claw-NexusClaw] Generated INBOX_WEBHOOK_SECRET");
}

const port = process.env.CLAW_SETUP_PORT?.trim();
if (port) {
  upsert("PORT", port);
  console.log(`[Claw-NexusClaw] Set PORT=${port}`);
}
...
if (openclaw) {
  ...
  console.log(`[Claw-NexusClaw] Set OPENCLAW_CONFIG=${normalized}`);
}
...
echo "[Claw-NexusClaw] Installing AGENTS.md orchestration rules..."
pnpm setup -- "${SETUP_ARGS[@]}"

echo
echo "[Claw-NexusClaw] Setup complete."
echo "Frontend: http://127.0.0.1:5173"
echo "API:      http://127.0.0.1:${PORT_TO_USE}/healthz"

if [[ "${START_AFTER_SETUP}" == "1" ]]; then
  echo "[Claw-NexusClaw] Starting development server..."
  exec pnpm dev:local
fi
