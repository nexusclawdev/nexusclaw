...
const TEMPLATE_PATH = path.join(__dirname, "..", "templates", "AGENTS-NexusClaw.md");
...
function findAgentsPath() {
  ...
  // Default target: current project root (claw-NexusClaw users first).
...
function main() {
  ...
  console.log(`[Claw-NexusClaw] Setting up orchestration rules`);
  console.log(`[Claw-NexusClaw] Target: ${agentsPath}`);
  console.log(`[Claw-NexusClaw] Port: ${port}`);
  ...
  if (existingContent.includes(START_MARKER) && existingContent.includes(END_MARKER)) {
    ...
    console.log(`[Claw-NexusClaw] Updated existing orchestration rules in ${agentsPath}`);
    console.log(`[Claw-NexusClaw] Done!`);
    return;
  }
  ...
  fs.writeFileSync(agentsPath, newContent, "utf8");
  console.log(`[Claw-NexusClaw] Orchestration rules added to top of ${agentsPath}`);
  console.log(`[Claw-NexusClaw] Your existing AGENTS.md content is preserved below.`);
  console.log(`[Claw-NexusClaw] Done!`);
}
