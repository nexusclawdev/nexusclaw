<!-- BEGIN NexusClaw orchestration rules -->
# NexusClaw Orchestration Rules

> This section defines how the AI agent handles task orchestration and directives.
...
**Messages starting with `$` are Claw-NexusClaw CEO Directives.**
...
- KO: `✅ Claw-NexusClaw 업무지시 전달 완료` (회의 진행) / `✅ Claw-NexusClaw 업무지시 전달 완료 (회의 생략)` (회의 없이)
...
2. Show the target AGENTS path (`~/.nexusclaw/workspace/AGENTS.md` by default, or response field).
...
The Claw-NexusClaw server detects the `$` prefix and automatically:
...
1. **`$` directive**: After collecting required meeting/path inputs and sending to API, reply with only `✅ Claw-NexusClaw 업무지시 전달 완료` (or language equivalent). Nothing else.
...
<!-- END claw-NexusClaw orchestration rules -->
